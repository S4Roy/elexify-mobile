...
export default function HomeScreen() {
  ...
  const deliveryLabel = defaultAddress
    ? [defaultAddress.addressLine1, defaultAddress.city.name, defaultAddress.state.name].filter(Boolean).join(', ')
    : 'Choose delivery address';
  const sections = [...(home.data ?? [])];
  const heroIndex = sections.findIndex(section => section.type === 'hero');
  const categoryIndex = sections.findIndex(section => section.type === 'category_section');
  ...
  return (
    <View style={shop.page}>
      ...
      <Animated.FlatList
        ...
        ListHeaderComponent={(home.isPending || home.error) ? (
